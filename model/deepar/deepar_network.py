...
from model import weighted_average
from modules import DistributionOutput, MeanScaler, NOPScaler, FeatureEmbedder
...
class DeepARNetwork(nn.Module):
    ...
    def unroll_encoder(
        ...
        ) # [Note] Generate sequences for all lags        
...
class RolledDeepARTrainingNetwork(DeepARNetwork):
    def forward(
        ...
        future_samples = []
        losses = []
        self.shifted_lags = [l - 1 for l in self.lags_seq] # [Note] prediction for1-step forward ( start from 0-lag
...
class DeepARPredictionNetwork(DeepARNetwork):
    ...
    def __init__(self, num_parallel_samples: int = 100, **kwargs) -> None:
        ...
        self.shifted_lags = [l - 1 for l in self.lags_seq] # [Note] prediction for 1-step forward ( starts from 0-lag)
    ...
    def sampling_decoder(
        ...
        for k in range(self.prediction_length):
            ...
            # (batch_size * num_samples, 1, *target_shape)
            new_samples = distr.sample() # [Note] sampling
